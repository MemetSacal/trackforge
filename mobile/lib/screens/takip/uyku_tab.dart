...
class _UykuTabState extends ConsumerState<UykuTab> {
  ...
  Future<void> _save(Map<String, dynamic>? existing) async {
    ...
    try {
      ...
      // ── FIX: önce loading'i kapat, sonra invalidate et ──
      // _initialized sıfırlama YOK — kullanıcının seçtiği değerler korunsun
      // invalidate yeni veriyi çeker ama _initialized=true olduğu için override etmez
    ...
    // finally kaldırıldı — setState ikiye ayrıldı, çakışma yok
  }
  ...
  Widget build(BuildContext context) {
    ...
    return sleepAsync.when(
      ...
      data: (sleepLog) {
        if (sleepLog != null && !_initialized && !_isLoading) {
          ...
          _initialized = true;  // artık bir daha override etme
        }
              ...
              Container(
                ...
                child: Column(
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                sleepLog != null ? 'Dün gece' : 'Planlanan',
                              ...
                              Text(
                                dur >= 7
                                    ? '✔ iyi kalite · $_qualityScore/10'
                                    : '⚠ yetersiz · $_qualityScore/10',
                                style: TextStyle(fontSize: 13, color: dur >= 7 ? positive : danger),
                              ),
                            ],
                          ),
                        ),
                        // ── FIX: Gauge sabit boyut + hizalama düzeltildi ──
                        SizedBox(
                          width: 100,
                          height: 60,
                          child: CustomPaint(
                    ...
                    // ── FIX: Grid aspect ratio artırıldı, metin artık taşmıyor ──
                    GridView.count(
                      shrinkWrap: true,
                    ...
                    Row(
                      children: [
                        Expanded(child: _TimeCard(
                            label: 'Uyudum', icon: '🌙',
                            time: _sleepTime,
                            bgSoft: bgSoft, border: border,
                            text: text, muted: muted,
                            onTap: () => _pickTime(true))),
                        const SizedBox(width: 10),
                        Expanded(child: _TimeCard(
                            label: 'Kalktım', icon: '☀️',
                            time: _wakeTime,
                            bgSoft: bgSoft, border: border,
                            text: text, muted: muted,
...
// ── FIX: Gauge painter — cy artık height'a bağlı değil ──
class _SleepGauge extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final radius = size.width * 0.38;  // sabit oran
    final cy     = size.height * 0.95; // taşmaz
  ...
  @override
  bool shouldRepaint(_SleepGauge o) =>
      o.progress != progress ||
      o.track    != track    ||
      o.fill     != fill;
}
...
class _TimeCard extends StatelessWidget {
  ...
  const _TimeCard({
    ...
    required this.bgSoft, required this.border,
